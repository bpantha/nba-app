...
export default function LazyTable({
  ...
  useEffect(() => {
    fetch(`${route}?page=${page}&page_size=${pageSize}`)
      .then((res) => res.json())
      .then((resJson) => console.log(resJson) || setData(resJson));
  }, [route, page, pageSize]);
          ...
          {data.map((row, idx) => (
            <TableRow key={idx}>
              ...
              {columns.map((col, idx2) => (
                <TableCell key={columns[idx2].headerName}>
                  ...
                  {columns[idx2].renderCell
                    ? columns[idx2].renderCell(row)
                    : defaultRenderCell(columns[idx2], row)}
                </TableCell>
              ))}
            </TableRow>
          ))}
